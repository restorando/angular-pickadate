/*
 * Orignal code supplied via GitHub: https://github.com/restorando/angular-pickadate
 * 
 * Depends on `date-helper.js`
 * available here:
 */
;
(function (angular) {
    ...
    .directive('pickadate', ['$locale', 'pickadateUtils', 'pickadateI18n', 'dateFilter', function ($locale, dateUtils, i18n, dateFilter) {
        return {
            ...
            template: '<div class="pickadate">' +
                ...
                '</ul>' +
                '<ul class="pickadate-cell">' +
                '<li ng-repeat="d in dates" ng-click="setDate(d)" class="{{d.className}}" ng-class="{\'pickadate-active\': date.strftime(\'%Y-%m-%d\') == d.dateObj.strftime(\'%Y-%m-%d\')}">' +
                '{{d.dateObj | date:"d"}}' +
                '</li>' +
                '</ul>' +
                '</div>' +
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                scope.render = function (initialDate) {
                    ...
                    for (var i = 0; i < allDates.length; i++) {
                        var className = "",
                            dateObj = allDates[i].toMidnight(),
                            date = dateFilter(dateObj, 'yyyy-MM-dd');

                        if (date < scope.minDate.strftime('%Y-%m-%d') || date > scope.maxDate.strftime('%Y-%m-%d') || dateFilter(dateObj, 'M') !== currentMonth.toString()) {
                ...
                scope.setDate = function (dateObj) {
                    ...
                    ngModel.$setViewValue(new Date(dateObj.date));
                };
